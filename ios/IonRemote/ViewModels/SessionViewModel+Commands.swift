    ...
    func send(_ command: RemoteCommand, intent: SendIntent = .automaticEssential) {
        ...
        case .userInitiated:
            guard let transport else {
                DiagnosticLog.log("CMD: dropped (no transport) intent=userInitiated state=\(connectionState)")
                // Suppress the toast mid-reconnect: transient transport gaps
                // during connecting/reconnecting are expected and self-heal.
